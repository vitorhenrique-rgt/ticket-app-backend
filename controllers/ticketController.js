...
exports.createTicket = async (req, res) => {
  ...
  try {
    ...
    const ticket = await Ticket.create(
      { title, description, requesterId, adminId, companyId, status },
      { transaction: t }
    );
    ...
    res
      .status(201)
      .json({ ticket, message: "Ticket created and associated with tag" });
  } catch (error) {
...
exports.getTicketStats = async (req, res) => {
  ...
  switch (filter) {
    ...
    default:
      return res
        .status(400)
        .json({ error: 'Filtro inválido. Use "day", "week" ou "month".' });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erro ao buscar estatísticas de tickets." });
  }
};
